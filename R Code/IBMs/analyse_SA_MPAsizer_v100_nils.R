...
species.order <- order(species.groups,species.names) # determine order of species to plot
...
# specify colours and line types to use for different species
ltys <- c(1:ngroups[1],1:ngroups[2])
cols <- c(rep('black',ngroups[1]),rep('blue',ngroups[2])) # specify colours
...
for (plotnum in 1:length(fmortcols)){
  lfmorts[1,,plotnum] <- returndat[[1]]$mean_Fmortality[,fmortcols[plotnum]]

  tiff(paste0(scenario.results.folder, scenario.name,'_Lifetime_mortality_F',
  ...
  if(length(returndat)>1){
    for (s in 1:nspecies) {
     lfmorts[s,,plotnum] <- returndat[[s]]$mean_Fmortality[,fmortcols[plotnum]]
...
for (plotnum in 1:length(fmortcols)){
  ...
  ylim <- ceiling(fmortplot[plotnum]*100)/100 #ceiling(-log(fmortplot[plotnum])*10)/10 # get maximum according to specs (turned into instantaneous mortality)
  #print(ylim) 
  
  foffs[1,,plotnum] <- returndat[[1]]$mean_Foffset[,fmortcols[plotnum]]
  foffs[1,foffs[1,,plotnum]>1,plotnum] <- 1 # cap to mean risk of 1
  tiff(paste0(scenario.results.folder, scenario.name,'_Fmortality_risk_F',fmortplot[plotnum]*100,'_',xlim,'km.tiff'), 
  ...
  plot(MPAsizes/1000,foffs[1,,plotnum],
       ...
       xlab = "MPA size (km)", ylab = "Annual fishing mortality",
       main = paste0(fmortplot[plotnum]*100, '% Annual fishing mortality'),
       col=cols[1],lty=ltys[1])
  axis(side = 1, at = seq(0,xlim,xlim/10))

  if(length(returndat)>1){
   for (s in 1:nspecies) {
    foffs[s,,plotnum] <- returndat[[s]]$mean_Foffset[,fmortcols[plotnum]]
    foffs[s,foffs[s,,plotnum]>1,plotnum] <- 1
...
ylim <- 1
for (plotnum in 1:length(fmortcols)){
  fsurvs[1,,plotnum] <- returndat[[1]]$mean_survival_probability[,fmortcols[plotnum]]
  fsurvs[1,1,plotnum] <- 1-fmortplot[plotnum]
  tiff(paste0(scenario.results.folder, scenario.name,'_Psurvival_F',fmortplot[plotnum]*100,'_',xlim,'km.tiff'), 
