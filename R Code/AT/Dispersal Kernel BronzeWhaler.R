...
#sp_receivermet <- paste0(datafolder,"Bronze _ Dusky sharks/WBG _ BTW station information.csv")
sp_tagmet <- paste0(datafolder,"Bronze _ Dusky sharks/Metadata_2021-08-10.csv")
#sp_meas <- paste0(datafolder,"Western blue groper and bluethroat wrasse/IMOS_animal_measurements.csv")
...
# What are the Id's of the bronze whalers
BronzeIDs <- sp_tagmet_dat %>%
  filter(Species2=="Carcharhinus brachyurus") %>%
  rename(transmitter_id=Code2) %>%
  select(transmitter_id) 

# Extract only the variables we are interested renaming them to remora format
d.dplyr <- sp_det_dat0 %>% 
  rename(transmitter_id=TagCode,
         detection_datetime=Timestamp.UTC,
...
Dispersal_Timescales_BronzeWhaler <- list(Daily=dispersal_summary_day,
                                     Weekly=dispersal_summary_week,
                                     Monthly=dispersal_summary_month)
saveRDS(Dispersal_Timescales_BronzeWhaler, file = "Data/Dispersal_Timescales_BronzeWhaler.RDS") # Save to github

##############################################


# Compute a histogram of distance per week
disp.hist <- Dispersal_Timescales_BronzeWhaler$Weekly %>%
  ggplot(aes(maxDistkm)) + geom_histogram() +theme_bw()
# Compute a histogram of num receiver stations
stat.hist <- Dispersal_Timescales_BronzeWhaler$Weekly %>%
