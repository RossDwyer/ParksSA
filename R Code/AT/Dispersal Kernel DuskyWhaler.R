...
# What are the Id's of the dusky whalers
DuskyIDs <- sp_tagmet_dat %>%
  filter(Species2=="Carcharhinus obscurus") %>%
...
Dispersal_Timescales_DuskyWhaler <- list(Daily=dispersal_summary_day,
                                     Weekly=dispersal_summary_week,
                                     Monthly=dispersal_summary_month)
saveRDS(Dispersal_Timescales_DuskyWhaler, file = "Data/Dispersal_Timescales_DuskyWhaler.RDS") # Save to github

##############################################


# Compute a histogram of distance per week
disp.hist <- Dispersal_Timescales_DuskyWhaler$Weekly %>%
  ggplot(aes(maxDistkm)) + geom_histogram() +theme_bw()
# Compute a histogram of num receiver stations
stat.hist <- Dispersal_Timescales_DuskyWhaler$Weekly %>%
