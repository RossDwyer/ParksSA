...
sp_receivermet <- paste0(datafolder,"Western blue groper and bluethroat wrasse/WBG _ BTW station information.csv")
sp_tagmet <- paste0(datafolder,"Western blue groper and bluethroat wrasse/IMOS_transmitter_deployment_metadata.csv")
...
# What are the Id's of the blue groper
BlueGropIDs <- sp_tagmet_dat %>%
  filter(species_common_name=="Western Blue Groper") %>%
...
Dispersal_Timescales_BlueGroper <- list(Daily=dispersal_summary_day,
                                     Weekly=dispersal_summary_week,
                                     Monthly=dispersal_summary_month)
saveRDS(Dispersal_Timescales_BlueGroper, file = "Data/Dispersal_Timescales_BlueGroper.RDS") # Save to github

##############################################


# Compute a histogram of distance per week
disp.hist <- Dispersal_Timescales_BlueGroper$Weekly %>%
  ggplot(aes(maxDistkm)) + geom_histogram() +theme_bw()
# Compute a histogram of num receiver stations
stat.hist <- Dispersal_Timescales_BlueGroper$Weekly %>%
