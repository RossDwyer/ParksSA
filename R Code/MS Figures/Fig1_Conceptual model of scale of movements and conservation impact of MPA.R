## R code used to fit the neg bin distribution in our conceptual figure. 
## Amended original to plot as L graphs and rotated y axis labs
## Use skewed normal to fit to empirical data based on mean +- SD.
...
jpeg(file ="Images/Figure 1 Conceptual.jpg",
     quality=100, bg="white", res=200, 
     height=2300, width=2500,pointsize=18) 
...
plot.labs <- rbind(c("A","B",""),c("C","D","E"),c("F","G","H"),c("I","J","K"))
...
for(xplot in 1:nrows){
  ...
  hnb <- hist(hist.metrics[xplot,],xlim = c(0,xlim.plot),n=max(distances)/hist.resolution,
              ...
              ylab = 'Relative Frequency',xlab=xlabs1[xplot],
              main=NULL,yaxt="n",bty="l")
  ...
  text(0.5*xlim.plot/100,0.95 * max(hnb$density),labels=plot.labs[xplot,1],cex=1.2)
  
  #} else if (xplot==2){
  plin <- plot(distances/10,protection.metrics[xplot,],type='l',lwd=2,
               bty="l",ylim = c(0,100),xlim=c(0,xlim.plot),
               ylab = 'Survival (% individuals)',xlab=xlabs[xplot],
               main=NULL,yaxt="n")
  axis(side = 2,las = 2) ## Rotate the y labels
  abline(v=median.metrics[xplot]/10,col='red',lty=2,lwd=2)
  text(0.5*xlim.plot/100,95,labels=plot.labs[xplot,2],cex=1.2)
  ...
  if(xplot==1){
    ...
    dlin <- plot(distances/10,difference.metrics[xplot,],type='l',lwd=2,
                 bty="l",xlim=c(0,xlim.plot),
                 ylab = 'Difference in survival',xlab=xlabs[xplot],
                 main=NULL, yaxt = "n") # Dont draw on y labs
    text(0.5*xlim.plot/100,0.95*max(difference.metrics[xplot,]),labels=plot.labs[xplot,3],cex=1.2)
    axis(side = 2,las = 2)## Rotate the y labels.
